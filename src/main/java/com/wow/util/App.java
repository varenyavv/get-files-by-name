...
public class App {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (dirCreated) {
            ...
            System.out.println("Output directory already exists. Please delete it and run this utility again");
        }

    }


    public static void getAllFilesContainingName(File directory, String fileToSearch, File outputDir) throws IOException {
        ...
        if (null != fList) {
            for (File file : fList) {
                if (file.isFile() && file.getName().toLowerCase().contains(fileToSearch.toLowerCase())) {
                    ...
                    Files.copy(file.toPath(), new File(outputDir, file.getParentFile().getName() + "-" + file.getName()).toPath(), StandardCopyOption.REPLACE_EXISTING);
